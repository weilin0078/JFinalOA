 ...
 * @qq 439635374
 */
package com.pointlion.sys.mvc.admin.cms;
...
import com.pointlion.sys.mvc.common.model.CmsType;
...
public class CmsController extends BaseController {
	...
	public void getListPage(){
		setBread("内容管理",this.getRequest().getServletPath()+"?type="+(StrKit.isBlank(getPara("type"))?"":getPara("type")),"管理");
		setAttr("type", (StrKit.isBlank(getPara("type"))?"":getPara("type")));
		render("/WEB-INF/admin/cms/list.html");
    }
	/***
	 * 获取列表
	 */
	...
	public void getEditPage(){
		...
		String type = (StrKit.isBlank(getPara("type"))?"":getPara("type"));
		setBread("内容维护",parentPath+"getListPage?type="+type,"内容维护");
		//是否是查看详情页面
		...
		setAttr("type", type);
		render("/WEB-INF/admin/cms/edit.html");
	}
	/***
	 * 保存
	 */
	...
	public void getReceiveListPage(){
		String parentPath = this.getRequest().getServletPath().substring(0,this.getRequest().getServletPath().lastIndexOf("/")+1);
		setBread("内容维护",parentPath+"getListPage?type=4","认领情况");
		String id = getPara("id");
		...
		setAttr("list", list);
		render("/WEB-INF/admin/cms/receiveList.html");
	}
	/*****************管理通知公告结束*********************************/
	...
	public void view(){
		...
		render("/WEB-INF/admin/cms/view.html");
	}
