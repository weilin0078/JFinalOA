...
public class ChineseNameUtil {
	...
	public static void generateUser(){
		List<SysOrg> list = SysOrg.dao.find("select * from sys_org");
    	for(SysOrg org : list){
    		if(org.getName().indexOf("部")<0||org.getName().indexOf("办")<0){
    			org.setName(ChineseNameUtil.getChinese()+ChineseNameUtil.getChinese()+ChineseNameUtil.getChinese()+"部");
        		org.update();
    		}
    		for(int i=0;i<30;i++){
    			SysUser u = new SysUser();
    			String name = ChineseNameUtil.getChineseName();
    			String username = PinYinUtil.getPingYin(name);
    			SysUser olu = SysUser.dao.getByUsername(username);
    			if(olu==null){
					...
					role.setKey("commonUser");
					role.setName("普通员工");
    				WorkFlowIdentityService.me.createRelationShip(u, role);
    				u.save();
    			}
    		}
    	}
