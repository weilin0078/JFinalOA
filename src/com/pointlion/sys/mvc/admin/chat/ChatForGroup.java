...
public class ChatForGroup {

	// 	静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
    private  static final AtomicInteger onlineCount = new AtomicInteger(0);
    // 	concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。若要实现服务端与单一客户端通信的话，可以使用Map来存放，其中Key可以为用户标识
    private static CopyOnWriteArraySet<ChatForGroup> webSocketSet = new CopyOnWriteArraySet<ChatForGroup>();
    //	定义一个记录客户端的聊天昵称
    ...
    public  ChatForGroup() {
        nickname = "好友："+onlineCount.getAndIncrement(); 
    }
    /*
	...
	*/
    ...
    private void broadcast(String info){
        for(ChatForGroup w:webSocketSet){
            try {
                synchronized (ChatForGroup.class) {
